...
% link frame projected on the base frame.
...
W1 = [zeros(3,1); (m1+m_motor)*g];
W2 = [zeros(3,1); (m2+m_motor)*g];
...
S_p1_1_3 = ComputeRigidBodyJ(COM_link_wrt_base{1 ,3}(:,:,2) - P1_wrt_base);
...
S_p2_1_3 = ComputeRigidBodyJ(COM_link_wrt_base{1 ,3}(:,:,2) - P2_wrt_base);

W1_3 = [0 ; 0 ; 0; F_ext_1_3_p1];

tau_1_3_a = - transpose(J{1,3}(:,:,2)) * transpose(S_p1_1_3) * W1_3;   %first question
tau_1_3_b = - transpose(J{1,3}(:,:,2)) * transpose(S_p2_1_3) * W1_3;   %second question
...
S_p3_1_4 = ComputeRigidBodyJ(COM_link_wrt_base{1 ,3}(:,:,1)-P3_wrt_base);

W1_4_a = [zeros(3,1); F_ext_1_4_p3];
W1_4_b = [M_ext_1_4; zeros(3,1)];

tau_1_4 = - (transpose(J{1,4}(:,:,1)) * transpose(S_p3_1_4) * W1_4_a + transpose(J{1,4}(:,:,2)) * W1_4_b);
...
S_p2_1_5 = ComputeRigidBodyJ(COM_link_wrt_base{1 ,5}(:,:,2) - P2_wrt_base);

% Computing the rigid body Jacobian for point P3 in the current configuration.
P3_wrt_base= T{1,5}(:,:,1) * [P3_wrt_f1;1];
S_p3_1_5 = ComputeRigidBodyJ(COM_link_wrt_base{1 ,5}(:,:,1) - P3_wrt_base);


W1_5_p2 = [zeros(3,1); F_ext_1_5_p2 ];
W1_5_p3 = [zeros(3,1); F_ext_1_5_p3 ];

tau_1_5 = - (transpose(J{1,5}(:,:,1)) * transpose(S_p3_1_5) * W1_5_p3 + transpose(J{1,5}(:,:,2)) * transpose(S_p2_1_5) * W1_5_p2) - (transpose(J{1,5}(:,:,1)) * W1 + transpose(J{1,5}(:,:,2)) * W2);
...
S_p1_2_3 = ComputeRigidBodyJ(COM_link_wrt_base{2, 3}(:,:,2) - P1_wrt_base);

W2_3 = [0 ; 0 ; 0; F_ext_2_3_p1];

tau_2_3 = - transpose(J{2,3}(:,:,2)) * transpose(S_p1_2_3) * W2_3;

%% Exercise 2.4

F_ext_2_4_p2 = [-0.8 ; -0.2; 0];    %N, acting on P2
M_ext_2_4_p1 = [0; 0; 500];         %N*mm, acting on P1
P2_2_wrt_base = T{2,4}(:,:,2)*[P2_2_wrt_f2;1];
% Computing the rigid body Jacobian for point P1 in the current configuration.
S_p2_2_4 = ComputeRigidBodyJ(COM_link_wrt_base{2, 4}(:,:,2)-P2_2_wrt_base);

W2_4_p2 = [0 ; 0 ; 0; F_ext_2_4_p2];
W2_4_p1 = [M_ext_2_4_p1; zeros(3,1)];

tau_2_4 = - (transpose(J{2,4}(:,:,2))*transpose(S_p2_2_4) * W2_4_p2 + transpose(J{2,4}(:,:,2))*W2_4_p1) ;

%% Exercise 2.5

F_ext_2_5_p1 = [0.5 ; -0.6; 0]; %N, acting on P1
F_ext_2_5_p2 = [1.0 ; -0.4; 0]; %N, acting on P2
P2_2_wrt_base = T{2,5}(:,:,2)*[P2_2_wrt_f2;1];

% Computing the rigid body Jacobian for point P1 in the current configuration.
S_p1_2_5 = ComputeRigidBodyJ(COM_link_wrt_base{2, 5}(:,:,2) - P1_wrt_base);
% Computing the rigid body Jacobian for point P2 in the current configuration.
S_p2_2_5 = ComputeRigidBodyJ(COM_link_wrt_base{2, 5}(:,:,2) - P2_2_wrt_base);

W2_5_p1 = [0 ; 0 ; 0; F_ext_2_5_p1];
W2_5_p2 = [0 ; 0 ; 0; F_ext_2_5_p2];

tau_2_5 = - (transpose(J{2,5}(:,:,2)) * transpose(S_p2_2_5) * W2_5_p2 + transpose(J{2,5}(:,:,2)) * transpose(S_p1_2_5) * W2_5_p1 + transpose(J{2,5}(:,:,2)) * W2) ;
%%